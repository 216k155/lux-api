...
var genesisTXID = 'e08ae0cfc35a1d70e6764f347fdc54355206adeb382446dd54c32cd0201000d3';
var CONCURRENCY = 10;
...
var bitcore = require('lux-core-api'),
...
var encodedData = require('soop').load('lux-core-api/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('lux-core-api/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('lux-core-api/lib/Address', {
...
TransactionDb.prototype._batch = function (script, cb) {
  db.batch(script, cb);
}
...
TransactionDb.prototype.add = function (tx, cb) {
  ...
  self._batch(dbScript, function (err) {
...
TransactionDb.prototype.putAddressInfo = function (addrInfo, cb) {
  var k = BAL_PREFIX + addrInfo.address;
  db.put(k, addrInfo.balance, function (err) {
    return cb(err);
  });
};

TransactionDb.prototype.getAddressInfo = function (address, cb) {
  var k = BAL_PREFIX + address;
  db.get(k, function (err, val) {
    var addrInfo = {};
    addrInfo.address = address;
    if (err) return cb(err);
    addrInfo.balance = val;
    return cb(err, addrInfo);
  });
};

TransactionDb.prototype.getRanking = function (cb) {
  var k = RANK_PREFIX;
  ...
  db.createReadStream({
    ...
    .on('data', function (data) {
      var kk = data.key.split('-');
      var vv = data.value.split(':');
      var idx = parseInt(kk[1]);
      var bal = parseFloat(vv[1]) / parseFloat(bitcore.util.COIN);
      ranking[idx] = {
        index: idx,
        address: vv[0],
...
TransactionDb.prototype._getRanking = function (cb) {
  var k = RANK_PREFIX;
  var ranking = new Array();
  var addrList = [];
  db.createReadStream({
    ...
    .on('data', function (data) {
      ...
      ranking[idx] = {
        address: vv[0],
        balance: parseInt(vv[1])
      };
      addrList.push(vv[0]);
    })
    .on('end', function () {
      return cb(ranking, addrList);
    });
}

TransactionDb.prototype.updateRanking = function (addrInfo, cb) {
  var self = this;
  var kLast = RANK_PREFIX + RANKING_MAX;
  db.get(kLast, function (err, val) {
    if (err && err.notFound) {
      var dbScript = [];
      dbScript.push({
        type: 'put',
        key: RANK_PREFIX + '0',
        value: addrInfo.address + ':' + addrInfo.balanceSat
      });
      for (var i = 1; i < RANKING_MAX + 1; i++) {
        dbScript.push({
          type: 'put',
          key: RANK_PREFIX + i,
          value: 'UgmHeCq7SGpkfJoPThqXVouzxwiLCYsuiG:0'
        });
      }
      return self._batch(dbScript, cb);
    }
    if (err) return cb(err);
    var vv = val.split(':');
    var bLast = parseInt(vv[1]);
    self._getRanking(function (ranking, addrs) {
      if (addrs.indexOf(addrInfo.address) < 0) {
        if (bLast >= addrInfo.balanceSat) return cb(err);
        ranking[RANKING_MAX] = {
          address: addrInfo.address,
          balance: addrInfo.balanceSat
        };
      } else {
        for (var i = 0; i < RANKING_MAX + 1; i++) {
          if (ranking[i].address == addrInfo.address) {
            ranking[i].balance = addrInfo.balanceSat;
            break;
          }
        }
      }
      ranking.sort(function (aa, bb) {
        if (aa.balance < bb.balance) return 1;
        if (aa.balance > bb.balance) return -1;
        return 0;
      });
      console.log("**************************************");
      ...
      for (var i = 0; i < RANKING_MAX + 1; i++) {
        dbScript.push({
          type: 'put',
          key: RANK_PREFIX + i,
          value: ranking[i].address + ':' + ranking[i].balance
        });
      }
      return self._batch(dbScript, cb);
    });
...
TransactionDb.prototype.updateRankingEx = function (addresses, cb) {
  var self = this;
  var kLast = RANK_PREFIX + RANKING_MAX;
  db.get(kLast, function (err, val) {
    if (err && err.notFound) {
      var dbScript = [];
      addresses.sort(function (aa, bb) {
        if (aa.balanceSat < bb.balanceSat) return 1;
        if (aa.balanceSat > bb.balanceSat) return -1;
        return 0;
      });
      var idx = 0;
      addresses.forEach(function (a) {
        dbScript.push({
          type: 'put',
          key: RANK_PREFIX + idx,
          value: a.address + ':' + a.balanceSat
        });
        idx += 1;
      });

      for (var i = idx; i < RANKING_MAX + 1; i++) {
        dbScript.push({
          type: 'put',
          key: RANK_PREFIX + i,
          value: 'UgmHeCq7SGpkfJoPThqXVouzxwiLCYsuiG:0'
    ...
    self._getRanking(function (ranking, addrs) {
      addresses.forEach(function (a) {
        if (addrs.indexOf(a.address) < 0) {
          if (a.balanceSat > bLast) {
            ranking.push({
              address: a.address,
          ...
          for (var i = 0; i < ranking.length; i++) {
            if (ranking[i].address == a.address) {
              ranking[i].balance = a.balanceSat;
              break;
            }
          }
        }
