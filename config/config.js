...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.lux');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\lux\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/lux/';
  if (isLinux) dataDir = process.env.HOME + '/.lux/';
}
...
var bitcoindConf = {
  ...
  user: process.env.BITCOIND_USER || 'lux',
  pass: process.env.BITCOIND_PASS || 'luxapi1612',
